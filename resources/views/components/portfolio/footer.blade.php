...
@php
    ...
    $socials = collect($profile?->social_links ?? [])
        ->map(function (array $social) {
            ...
            $key = match (true) {
                str_contains($platform, 'github') => 'github',
                str_contains($platform, 'linkedin') => 'linkedin',
                str_contains($platform, 'discord') => 'discord',
                str_contains($platform, 'twitter'), str_contains($platform, 'x.com'), $platform === 'x' => 'x',
                default => 'link',
            };

            return [
                'key' => $key,
                'label' => $social['platform'] ?? ucfirst($key),
                'url' => $social['url'] ?? '#',
            ];
        })
...
<footer class="site-footer">
    <div class="site-footer__inner">
        ...
        {{-- Right — every link carries a visible text label, so the accessible
             name is the platform rather than the bare URL. --}}
        <div class="site-footer__socials">
            @foreach($socials as $social)
                <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer" class="site-footer__social">
                    ...
                    <span class="site-footer__social-label">{{ $social['label'] }}</span>
                </a>
            @endforeach
        </div>
